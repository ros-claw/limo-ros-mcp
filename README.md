...
- `limo_get_readiness` returns a compact hash-sealed `limo.readiness.v1` reference by default. The compatibility name `limo_get_patrol_readiness` is available with `--compat-tools` or `--profile full`.
- A bounded `ObservationHub` reuses read-only transports, caches fresh summaries, deduplicates concurrent readiness calls, and invalidates snapshots when the transport generation changes.
